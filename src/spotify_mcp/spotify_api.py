...
class Client:
    ...
    def _get_candidate_device(self):
        devices = self.get_devices()  # This line must be indented correctly

        if not devices:  # Check if the devices list is empty
            self.logger.error("No available devices. Please open Spotify on a device and try again.")
            raise ValueError("No active devices found. Open Spotify and play something first.") 
        ...
        self.logger.info(f"No active device, assigning {devices[0]['name']}.")
        return devices[0]  # Ensure this line is also properly indented
